...
A functional PhaC indicates PHA biosynthesis potential, but the polymer type (SCL vs. MCL) is constrained by the PhaC class and the monomer-supplying enzymes present. **A single decision layer (`pha_type`) is authoritative**: it determines the genomic potential, and the metabolic-pathway view is *derived from and gated by* it — a pathway is shown active only when PhaC is functional and `pha_type` reports a concrete potential, so the report never contradicts itself (e.g. it cannot show a pathway "active → MCL" while the potential layer abstains). The potential layer integrates the surviving enzymes with class biochemistry:
